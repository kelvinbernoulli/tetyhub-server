...
export const checkoutSchema = Joi.object({      
    ...
    phone_one:   Joi.string().trim().pattern(/^\+?[1-9]\d{1,14}$/).required().label("Phone"),
    phone_two:   Joi.string().trim().pattern(/^\+?[1-9]\d{1,14}$/).optional().allow(null, '').label("Phone (alt)"),
    ...
    zip_code:    Joi.string().trim().max(20).optional().allow(null, '').label("Zip Code"),
    coupon_code: Joi.string().trim().max(50).optional().allow(null, '').label("Coupon Code"),
    note:        Joi.string().trim().max(500).optional().allow(null, '').label("Order Note"),
