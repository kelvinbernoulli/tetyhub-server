...
export const getVendorOrders = async (req, res) => {
    try {
        ...
        const orders = await Order.fetchVendorOrders(req.auth.vendorId, {
            status,
            payment_status,
            offset,
            limit,
...
export const getVendorOrderById = async (req, res) => {
    try {
        ...
        const orders = await Order.getOrderById(
            orderId,
            null,
            req.auth.vendorId
        );
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const { error, value } = updateOrderStatusSchema.validate(body, {
            abortEarly: false,
            stripUnknown: true,
        });
        if (error) {
...
export const getDashboard = async (req, res) => {
    try {
        ...
        const { period } = query;
        const dashboard = await VendorDashboardModel.getDashboard(
            vendorId,
            period
        );
...
export const getRevenueChart = async (req, res) => {
    try {
        ...
        const chart = await VendorDashboardModel.getRevenueChart(
            vendorId,
            period
        );
        return respondWithSuccess(
...
export const getLowStockProducts = async (req, res) => {
    try {
        ...
        const { limit } = query;
        const products = await VendorDashboardModel.getLowStockProducts(
            vendorId,
            parseInt(limit) || 10
        );
        return respondWithSuccess(
